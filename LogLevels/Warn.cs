...
namespace ODDating.LogLevels
{
    public class Warn
    {
        public Warn(string message, bool logMassage = true, bool zennoLogMassage = true, bool nlogMassage = false)
        {
                string currentThreadId = Thread.CurrentThread.ManagedThreadId.ToString();
                string sample = $"{currentThreadId}|{DateTime.Now.ToLongTimeString()}|{typeof(Warn).Name.ToUpper()}| {message}" + Environment.NewLine;
                if (logMassage == true)
                {
                ...
                if (nlogMassage)
                {
                    ...
                    {
                        Logger Logger = LogManager.GetLogger("Warn");
                        Logger.Warn(message);
                    }
                }
        }
